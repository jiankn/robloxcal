...
const PRESET_CONFIGS = [
    { n: 8, m: 2, label: '8 chips / 2 bombs' },
    { n: 10, m: 3, label: '10 chips / 3 bombs' },
    { n: 12, m: 4, label: '12 chips / 4 bombs' },
    { n: 15, m: 3, label: '15 chips / 3 bombs' },
]

export default function BombChipPage() {
    const game = useGame()

    // 游戏参数
    const [n, setN] = useState(10)  // chips 数量
    const [m, setM] = useState(3)   // bombs 数量
    ...
    // 选择预设
    const selectPreset = (preset: typeof PRESET_CONFIGS[0]) => {
        setN(preset.n)
        setM(preset.m)
        reset()
    }
            ...
            <header className="relative overflow-hidden">
                ...
                <div className="relative max-w-5xl mx-auto px-4 py-12 text-center">
                    ...
                    <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">
                        {game.display_name} <span className="text-red-400">Calculator</span>
                    </h1>

                    <p className="text-zinc-400 max-w-xl mx-auto">
                        Input your opponent's recent choices and get AI-powered placement recommendations
                    </p>
                </div>
            </header>
                ...
                <div className="flex justify-center gap-3 mb-8 flex-wrap">
                    <Link href={`/${game.slug}/odds`}>
                        <Badge className="bg-orange-500/20 text-orange-300 border-orange-500/30 cursor-pointer hover:bg-orange-500/30">
                            <TrendingUp className="h-3 w-3 mr-1" /> Odds Calculator
                        </Badge>
                    </Link>
                        ...
                        <Card className="bg-zinc-900/50 border-zinc-800">
                            <CardHeader className="pb-3">
                                <CardTitle className="text-lg flex items-center gap-2">
                                    ...
                                    Record Opponent Choices
                                </CardTitle>
                                <CardDescription>
                                    Click positions your opponent chose, then submit
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                {/* Position Grid */}
                                ...
                                <div className="text-sm text-zinc-500">
                                    Selected: {currentRound.length} / {m} positions
                                </div>
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {history.slice(-5).map((round, i) => (
                                                <div key={i} className="text-xs bg-zinc-800 px-2 py-1 rounded">
                                                    R{history.length - 4 + i}: [{round.opponentChoices.join(', ')}]
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        <Card className="bg-gradient-to-br from-red-900/20 to-zinc-900/50 border-red-500/20">
                            <CardHeader>
                                ...
                                {recommendations.length > 0 && (
                                    <CardDescription>
                                        Estimated hit probability: <span className="text-green-400 font-bold">{(hitProbability * 100).toFixed(1)}%</span>
                                    </CardDescription>
                                )}
                            </CardHeader>
                            <CardContent>
                                {recommendations.length === 0 ? (
                                    <div className="text-center py-8 text-zinc-500">
                                        ...
                                        <p>Add opponent history to get recommendations</p>
                                    </div>
                                ) : (
                        ...
                        <Card className="bg-zinc-900/50 border-zinc-800">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-zinc-500 mt-3 text-center">
                                    Darker = more frequently chosen by opponent
                                </p>
                            </CardContent>
                        </Card>
                    </div>
