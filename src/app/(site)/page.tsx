  ...
  return (
    <>
      ...
      <section id="games" className="rc-container rc-section scroll-mt-20">
        ...
        {/* More Games 紧凑网格 - SEO: 确保所有游戏都有主页入站链接 */}
        {allGames.length > 4 && (
          <div className="mt-10">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white">More Games</h3>
              <Link href="/games" className="text-sm text-purple-400 hover:text-purple-300 flex items-center gap-1">
                View All
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
              {allGames
                .filter(g => !featuredGames.some(fg => fg.slug === g.slug))
                .map((game) => (
                  <Link
                    key={game.slug}
                    href={`/${game.slug}`}
                    className="group p-3 rounded-lg bg-zinc-900/50 border border-zinc-800/50 hover:border-purple-500/30 hover:bg-zinc-800/50 transition-all"
                  >
                    <div className="flex flex-col items-center text-center gap-2">
                      <GameLogo slug={game.slug} size={40} />
                      <span className="text-sm font-medium text-white group-hover:text-purple-300 truncate w-full">
                        {game.display_name}
                      </span>
                      <span className="text-[10px] text-zinc-500 truncate w-full">
                        {game.full_name.length > 15 ? game.full_name.substring(0, 13) + '...' : game.full_name}
                      </span>
                    </div>
                  </Link>
                ))}
              ...
              <Card className="glass-card border-zinc-800/50 hover-lift hover-glow transition-all hover:border-green-500/30">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <GameLogo slug={update.slug} size={40} />
                      <div>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            "@context": "https://schema.org",
            "@type": "ItemList",
            "name": "Roblox Game Calculators",
            "itemListElement": allGames.map((game, i) => ({
              "@type": "ListItem",
              "position": i + 1,
              "name": `${game.display_name} Calculator`,
